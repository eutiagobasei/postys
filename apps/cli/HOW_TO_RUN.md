...
This creates a global `postys` command you can use anywhere:
...
**After linking, you can use `postys` from any directory!**

## Option 3: Use pnpm Filter (From Root) 📦

```bash
# From the monorepo root
pnpm --filter postys start -- --help
pnpm --filter postys start -- posts:list
pnpm --filter postys start -- posts:create -c "Hello" -i "twitter-123"
```
...
echo 'export POSTYS_API_KEY=your_api_key' >> ~/.bashrc
...
- `POSTYS_API_KEY` - Your Postys API key (required for all operations)

### Optional

- `POSTYS_API_URL` - Custom API endpoint (default: `https://api.postys.io`)
...
```bash
export POSTYS_API_KEY=your_key
export POSTYS_API_URL=https://custom-api.com
```

**Permanent (add to shell profile):**
```bash
# For bash
echo 'export POSTYS_API_KEY=your_key' >> ~/.bashrc
source ~/.bashrc

# For zsh
echo 'export POSTYS_API_KEY=your_key' >> ~/.zshrc
source ~/.zshrc
```
...
| **Global Link** | `postys` (after `pnpm link --global`) | **Recommended** for development |
| **pnpm Filter** | `pnpm --filter postys start --` | From monorepo root |
| **npm Global** | `postys` (after `npm i -g postys`) | After publishing to npm |
| **npx** | `npx postys` | One-off usage without installing |
...
# 5. Start using!
postys posts:create -c "My first post" -i "twitter-123"
```

Now you can use `postys` from anywhere! 🚀
