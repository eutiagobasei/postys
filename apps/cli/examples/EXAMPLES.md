# Postys CLI - Advanced Examples

This directory contains examples demonstrating the full capabilities of the Postys CLI, including posts with comments and multiple media.

## Understanding the Post Structure

The Postys API supports a rich post structure:
...
```bash
postys posts:create --json examples/post-with-comments.json
```
...
```bash
postys posts:create --json examples/multi-platform-post.json
```
...
```bash
postys posts:create --json examples/thread-post.json
```
...
2. **Invalid image path** - Ensure images are accessible URLs or uploaded to Postys first
