...
The Postys CLI supports platform-specific settings for each integration. Different platforms have different options and requirements.
...
```bash
postys posts:create --json post-with-settings.json
```
...
```bash
postys posts:create --json reddit-post.json
```
...
```bash
postys posts:create --json youtube-video.json
```
